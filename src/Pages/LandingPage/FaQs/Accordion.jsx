...
const Accordion = ({ open, toggle, index, title, description, id }) => {
  return (
    ...
    >
      <span className=" text-4xl text-[#868e96] font-semibold max-[826px]:text-2xl">{id}</span>
      <div className=" flex flex-col gap-6 w-[500px] max-[826px]:w-[400px] max-[692px]:w-[250px] max-[443px]:w-[190px] max-[367px]:w-[150px] ">
        ...
        <p
          className={
            !open ? "text-gray-300 text-sm hidden max-[826px]:text-xs" : "text-gray-300 text-sm max-[826px]:text-xs"
          }
        >
          {description}
        </p>
      ...
      <span className="w-[50px] h-[50px] max-[826px]:w-[25px] max-[826px]:h-[25px] rounded-full bg-white flex items-center justify-center">
        {!open ? <AiOutlinePlus size={25} className=" max-[826px]:w-[15px]" /> : <AiOutlineMinus size={25} className=" max-[826px]:w-[15px]" />}
      </span>
    </div>
  );
};
